...
if [ $QUIET -eq 0 ]; then
    ...
    if [ ${#INVALID_KEYBOXES[@]} -gt 0 ]; then
        print_orange "  Total compromised and expired keyboxes: ${#INVALID_KEYBOXES[@]}"
    ...
    if [ ${#WARNING_KEYBOXES[@]} -gt 0 ]; then
        ...
        for kb in "${WARNING_KEYBOXES[@]}"; do print_light_yellow "    $kb"; done
    else
